...
- **New tests**, including coverage for existing code (Vitest is installed).
...
- **Dependency patch-version bumps** — *except* React, Vite, TypeScript, Tailwind, react-router-dom, and `@supabase/supabase-js` (framework-level deps need scrutiny even on patches).
...
- **Dependency minor or major bumps**, and any bump of: React, Vite, TypeScript, Tailwind, react-router-dom, vite-plugin-pwa, `@supabase/supabase-js`, `vitest`.
