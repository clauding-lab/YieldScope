...
28. **RESOLVED 2026-07-11 — the Macro CPI-components heatmap was REMOVED (see landmine 29).** It formerly keyed columns by month name and threw React "duplicate key" warnings via `buildCpiHeatmapCols`/`monthsFromSeries`; both are deleted along with the panel. Kept as a numbered tombstone so later landmine references don't shift.

29. **Panels REMOVED 2026-07-11 (de-demo batch) vs. what stays Demo BY DESIGN.** REMOVED (not demo — gone from the UI entirely): the LCR/NSFR tiles (Banking), Ways & Means + ADP (Fiscal — `non_nbr_tax_revenue`/`ways_means_usage_cr` never populated), the call-money intraday heatmap (Liquidity), the ALCO decision log (Intelligence), the Fiscal revenue-split donut, Banking NPL-by-segment + deposits-by-ownership donut + top-10-banks heatmap + NPL-trajectory-by-segment (no per-bank source — the dead "All 60 banks"/"Export" buttons went with the top-10 heatmap), the Fiscal-pressure composite gauge (methodology undefined), and Macro Core CPI + the CPI-components 8-month heatmap (no source). The Banking repo tile is no longer demo either — it's relabeled **"Interbank repo · volume"** and wired live to `interbank_repo_data` (consistent with landmine 19: it was never central-bank repo, so "Repo borrow from BB" was the wrong label all along; SLF-draw/BB-repo themselves stay retired per EconDelta PR #62). What genuinely REMAINS Demo BY DESIGN — don't try to "wire" this: the offline curve/auctions fixture fallbacks (no-credentials builds only). Backlog + rationale: `docs/econdelta-wishlist.md`.
